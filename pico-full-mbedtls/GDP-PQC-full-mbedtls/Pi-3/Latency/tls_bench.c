...
const char *server_cert_path = "./certs/dilithium_level2_entity_cert.pem";
const char *server_key_path = "./certs/dilithium_level2_entity_key.pem";
const char *client_cert_path = "./certs/dilithium_level2_root_cert.pem";
...
#ifndef NO_CERTS
    ret = wolfSSL_CTX_load_verify_locations(cli_ctx, client_cert_path, NULL);
...
#ifndef NO_CERTS
    ret = wolfSSL_CTX_use_PrivateKey_file(srv_ctx, server_key_path, 1);
    if (ret != WOLFSSL_SUCCESS) {
        fprintf(stderr, "error loading server key\n");
        goto exit;
    }

    ret = wolfSSL_CTX_use_certificate_file(srv_ctx, server_cert_path, 1);
    if (ret != WOLFSSL_SUCCESS) {
