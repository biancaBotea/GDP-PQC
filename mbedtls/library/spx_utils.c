                  ...
                  const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = 0; i < tree_height - 1; i++) {
        ...
        if (leafidx & 1) {
            md->thash(buffer + SPX_N, buffer, 2, pub_seed, addr);
            memcpy(buffer, auth_path, SPX_N);
        }
        else {
            md->thash(buffer, buffer, 2, pub_seed, addr);
            memcpy(buffer + SPX_N, auth_path, SPX_N);
        }
        auth_path += SPX_N;
    }
    ...
    set_tree_index(addr, leafidx + idx_offset);
    md->thash(root, buffer, 2, pub_seed, addr);
}
...
{
	unsigned char *stack = (unsigned char *)mbedtls_calloc((tree_height + 1) * SPX_N, sizeof(unsigned char));
	//unsigned char stack[(tree_height + 1)*SPX_N];
	unsigned int *heights = (unsigned int *)mbedtls_calloc((tree_height + 1), sizeof(unsigned int));
    ...
    for (idx = 0; idx < (uint32_t)(1 << tree_height); idx++) {
        ...
        gen_leaf(md, stack + offset*SPX_N,
