 ...
 */
static void wots_gen_sk(sphincs_md_info_t *md, unsigned char *sk, const unsigned char *sk_seed,
                        uint32_t wots_addr[8])
{
    ...
    /* Generate sk element. */
	md->prf_addr(sk, sk_seed, wots_addr);
}
 ...
 */
static void gen_chain(const sphincs_md_info_t *md, unsigned char *out, const unsigned char *in,
                      ...
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = start; i < (start+steps) && i < SPX_WOTS_W; i++) {
        ...
        md->thash(out, out, 1, pub_seed, addr);
    }
}
 ...
 */
void wots_gen_pk(const sphincs_md_info_t *md, unsigned char *pk, const unsigned char *sk_seed,
                 const unsigned char *pub_seed, uint32_t addr[8])
{
    uint32_t i;
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        ...
        wots_gen_sk(md, pk + i*SPX_N, sk_seed, addr);
        gen_chain(md, pk + i*SPX_N, pk + i*SPX_N,
               ...
               uint32_t addr[8])
{
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        set_chain_addr(addr, i);
        wots_gen_sk(md, sig + i*SPX_N, sk_seed, addr);
        gen_chain(md, sig + i*SPX_N, sig + i*SPX_N, 0, lengths[i], pub_seed, addr);
    }
}
                      ...
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        set_chain_addr(addr, i);
        gen_chain(md, pk + i*SPX_N, sig + i*SPX_N,
